...
try {
    ...
    if ($userId && $friendId) {
        ...
        if ($stmt->execute([$userId, $friendId])) {
            echo json_encode(['status' => 'success', 'message' => 'Friend request canceled successfully.']);
        } else {
            $errorInfo = $stmt->errorInfo();
            echo json_encode(['status' => 'error', 'message' => 'Failed to cancel friend request.', 'error' => $errorInfo[2]]);
        }
    } else {
